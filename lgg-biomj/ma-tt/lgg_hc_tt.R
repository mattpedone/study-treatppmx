...
HC.sum.all <- foreach(k = 1:K) %dopar%
  {
      train_pred <- data$pred[1:130,]
      train_prog <- data$prog[1:130,]
      train_yord <- data$yord[1:130]#-1
      train_ymat <- data$ymat[1:130,]
      train_trt <- data$trtsgn[1:130]#-1
...
HC.sum.all <- array(unlist(HC.sum.all), dim = c(n, 14, K))
  ...
  for(myrep in 1:nrep){  
    trtAPT <- data$trtsgn[131:158]#-1
    Rapp <- data$prog[131:158,]
    outcomAPT <- data$yord[131:158]#-1
    trtsgn <- data$trtsgn[131:158]
    myoutot <- data$ymat[131:158,]
    ...
    for (mysub in 1:n){
      ...
      totutAPT <- myyAPT*mycons1APT[130 + mysub, 131:158]               ### utility 
  ...
  ### summary meaures
  return<-crt1*p1+crt2*p2-sum(as.numeric(outcomAPT>=2))/28
  return
