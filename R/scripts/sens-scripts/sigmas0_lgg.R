...
for(idx1 in 1:length(vecsigma)){
  for(idx2 in 1:length(vecs0)){
    ...
    b <- vecs0[idx2]
    modelpriors <- list()
    modelpriors$hP0_m0 <- rep(0, ncol(Y)); modelpriors$hP0_L0 <- diag(a, ncol(Y))
    modelpriors$hP0_nu0 <- ncol(Y) + 2; modelpriors$hP0_V0 <- diag(b, ncol(Y))
