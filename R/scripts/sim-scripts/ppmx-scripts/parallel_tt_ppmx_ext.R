...
myres0 <- foreach(k = 1:K) %dopar%
  {
  X_train <- data.frame(simdata$pred[[k]][1:200,])
  Z_train <- data.frame(simdata$prog[[k]][1:200,])
  Y_train <- data.frame(simdata$ymat[[k]][1:200,])
  
  X_test <- data.frame(simdata$pred[[k]][201:228,])
  Z_test <- data.frame(simdata$prog[[k]][201:228,])
  Y_test <- data.frame(simdata$ymat[[k]][201:228,])
  
  trtsgn_train <- simdata$trtsgn[[k]][1:200]
  trtsgn_test <- simdata$trtsgn[[k]][201:228]
...
mywk1 <- myprob[[1]][201:228,]%*%wk
mywk2 <- myprob[[2]][201:228,]%*%wk
...
for(k in 1:K){
  ...
  myoutot <- simdata$yord[[k]][201:228,]
