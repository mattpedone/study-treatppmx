...
#for(sc in 10:12){
sc <- 1
simdata <- loadRData(paste0("data/scenext", sc, ".RData"))
mypath <- c(paste0("output/simulation-scenarios/train-test/scen-ext-", sc))
################################ Functions ########################################
...
HC.sum.all <- foreach(k = 1:K) %dopar%
  {
  train_pred <- simdata$pred[[k]][1:200,]
  train_prog <- simdata$prog[[k]][1:200,]
  train_yord <- simdata$yord[[k]][1:200]-1
  train_ymat <- simdata$ymat[[k]][1:200,]
  train_trt <- simdata$trtsgn[[k]][1:200]-1
  ...
  for (mysub in 1:n){
    ...
    con_clu <- ConsensusClusterPlus(t(s_train_pred),maxK=15,reps=100,pItem=0.90,pFeature=1,
...
rst.hc<-ConsensusClusterPlus(t(d),maxK=15,reps=100,pItem=0.90,pFeature=1,
                             clusterAlg="hc",distance="pearson", 
                             #clusterAlg="km",distance="euclidean", 
                             #clusterAlg="pam",distance="manhattan", 
                             seed=126);
...
for(myrep in 1:nrep){  
  trtAPT <- simdata$trtsgn[[myrep]][201:228]-1
  Rapp <- simdata$prog[[myrep]][201:228,]
  outcomAPT <- simdata$yord[[myrep]][201:228]-1
  trtsgn <- simdata$trtsgn[[myrep]][201:228]
  myoutot <- simdata$ymat[[myrep]][201:228,]
  ...
  for (mysub in 1:n){
    ...
    totutAPT <- myyAPT*mycons1APT[200 + mysub, 201:228]               ### utility 
...
for(my.pick in 1:nrep){
  ...
  outcomAPT <- simdata$yord[[my.pick]][201:228] - 1
