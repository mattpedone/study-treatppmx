...
load("output/simulation-scenarios/train-test/scen-alt-4/ma_hc_tt.RData");  
...
for(myrep in 1:nrep){  
  trtAPT <- scenalt4$trtsgn[[myrep]][125:152]-1
  Rapp <- scenalt4$prog[[myrep]][125:152,]
  outcomAPT <- scenalt4$yord[[myrep]][125:152]-1
  trtsgn <- scenalt4$trtsgn[[myrep]][125:152]
  myoutot <- scenalt4$ymat[[myrep]][125:152,]
...
for(my.pick in 1:nrep){
  ...
  outcomAPT <- scenalt4$yord[[my.pick]][125:152] - 1
  HCppCUT <- as.vector(countUT(case2HCppUT, outcomAPT))
  NPC[my.pick] <- HCppCUT
}

save(MOT, file="output/simulation-scenarios/train-test/scen-alt-4/mot_hc.RData")
save(MTUg, file="output/simulation-scenarios/train-test/scen-alt-4/mtug_hc.RData")
save(NPC, file="output/simulation-scenarios/train-test/scen-alt-4/npc_hc.RData")
