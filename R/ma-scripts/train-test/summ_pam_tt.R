...
load("output/simulation-scenarios/train-test/scen-alt-3/ma_pam_tt.RData");  
...
for(myrep in 1:nrep){  
  trtAPT <- scenalt3$trtsgn[[myrep]][125:152]-1
  Rapp <- scenalt3$prog[[myrep]][125:152,]
  outcomAPT <- scenalt3$yord[[myrep]][125:152]-1
  trtsgn <- scenalt3$trtsgn[[myrep]][125:152]
  myoutot <- scenalt3$ymat[[myrep]][125:152,]
...
for(my.pick in 1:nrep){
  ...
  outcomAPT <- scenalt3$yord[[my.pick]][125:152] - 1
  HCppCUT <- as.vector(countUT(case2HCppUT, outcomAPT))
  NPC[my.pick] <- HCppCUT
}

save(MOT, file="output/simulation-scenarios/train-test/scen-alt-3/mot_pam.RData")
save(MTUg, file="output/simulation-scenarios/train-test/scen-alt-3/mtug_pam.RData")
save(NPC, file="output/simulation-scenarios/train-test/scen-alt-3/npc_pam.RData")
