...
for(mysub in 1:n.mysub){
  ...
  myy<-matrix(0,nrow=select.sub.n,ncol=3);#151 x 3 (n training set )
  for (j in 1:select.sub.n){myy[j,outcom[j]+1]=1}#espande la y da vettore ordinale a matrice binaria
...
save(HC.sum.all,file="output/SimuCase2HCpp.rda")
